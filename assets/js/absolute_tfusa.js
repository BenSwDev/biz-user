function addOrder(data){
	...
	if(caltype > 0){
		...
		if(data.allDay == 1){
			...
			//console.log(caltype);
			writeslot  = '<div title="'+boxtitle+'" class="order ' + adminApproved + spaT + tGender + tGroup + lockedTherapist +'" data-parentOrder="'+data.parentOrder+'" data-orderid="' + orderID +'" data-orderidbysite="' + data.unitID + '" data-size="' + data.width + '" data-margin="' + data.right + '" style="width:' + data.width + '%;right:' + data.right + '%">';
...
function addShift_inOrders(data){
	...
	writeslot  = '<div class="shift '+_status+'" data-size="' + data.width + '" data-margin="' + data.right + '" style="width:' + data.width + '%;right:' + data.right + '%">' + addtoshift + (data.orderName? data.orderName : type_name)  + '</div>';
	slot.append(writeslot);
}
...
function startDrag(elm) {
	//debugger;
	$(elm.target).closest('.order').addClass('blap');
}
