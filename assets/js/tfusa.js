...
$.extend(addDayOrder.prototype, {
	...
	addUI:function(){
        ...
        if (self.orderType == 'preorder')
            clist.push('preorder');
		var setwidth = 'calc('+self._width().toFixed(2)*100+'% + '+self._width().toFixed(2)*2+'px)';
		var dataui = $('<div class="'+clist.join(' ')+'" data-orderID="'+self.orderID+'" data-size="'+setwidth+'" data-margin='+this.startTime.substr(3,5)+' data-orderidbysite="'+self.orderIDBySite+'" style="width:'+setwidth+';margin-right:'+this.startTime.substr(3,5) +'%"><div class="all"></div>\
