...
?>
<style>
   .mainSectionWrapper ul li {height: 36px;line-height: 1;vertical-align: middle;display:flex;align-items: center;}
    ...
    .mainSectionWrapper ul li.hasSub ul {
        margin-right: 20px;}
    .mainSectionWrapper ul li input , .mainSectionWrapper ul li label {display: inline-flex;line-height: 1;vertical-align: middle;margin-left: 7px;height: 60px;align-items: center;}
...
<div class="mainSectionWrapper">
    <h1 class="sectionName" style="text-align: right">מקורות הגעה</h1>
    <div>
        <button class="save saveList" style="padding: 0">שמור</button>
    </div>
    <div style="text-align: right">
        <div id="cpnsList">
            <b style="display:block; width:500px; border-bottom:1px solid #ccc; margin-top:20px; margin-bottom:10px">מקורות כלליים</b>
            <ul >
<?php
...
?>
                <li>
                    <input type="checkbox" id="src_<?=$item['id']?>" class="paytypescheckbox" name="source[<?=$item['id']?>]" value="1" <?=($item['active'] ? ' checked ' : '')?> <?=($item['mainActive'] ? '' : 'disabled')?> />
                    <label style="width:350px" for="src_<?=$item['id']?>"><?=$item['fullname']?></label>
                    <div class="invoiceDiv" style="display:inline-flex">
                        <input type="checkbox" id="showP_<?=$item['id']?>" class="paytypescheckbox" name="showP[<?=$item['id']?>]" value="1" <?=$item['showPrice'] ? ' checked ' : ''?> />
                        <label for="showP_<?=$item['id']?>" style="width:250px">הצג מחיר ללקוח</label>
                        <?php  /*<div class="autoInvoiceDiv" style="display:<?=($sitePayments[$item['key']]['invoice'] ? 'inline-block' : 'none')?>">
...
?>
            ...
            <b style="display:block; width:500px; border-bottom:1px solid #ccc; margin-top:20px; margin-bottom:10px">ספקים</b>
            <ul >
<?php
    foreach ($sources as $item) {
        if (!$item['payTypeKey'])
...
<script>
    $(document).ready(function () {
        ...
        $(".saveList").on("click",function(){
            var data = $('#cpnsList').find('input').serialize();
            $.post('ajax_sources.php', 'act=saveSrc&' + (data || '')).then(function(res){
