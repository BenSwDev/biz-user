...
<?php
    foreach(range(0, 30, 5) as $i)
        echo '<div class="timeout"><input type="radio" name="timeout" id="timeout' , $i , '" value="' , $i , '" ' , ($timeout == $i ? 'checked' : '') , '/> <label for="timeout' , $i , '">' , $i , " דק'</label></div>";
?>
            </div>
<table>
        <thead>
            ...
            <?foreach ($realTimes as $time) {?>
                <th><?=$time?> דקות<div class="pricetype"><span>יחיד</span><span>זוג</span><span>קבוצה</span></div></th>
            <?}?>
