...
$que = "SELECT orders.*, rooms_units.unitName
        ...
        WHERE orders.siteID = " . $siteID . " AND orders.parentOrder > 0 AND orders.parentOrder <> orders.orderID AND orders.status = 1 AND parent.status = 1 
...
?>
      <tr style="position:sticky;bottom:0;z-index:1;background:white">
         <td style="line-height:1">סה"כ מטפלים: <?=count($masters)?></td>
         <td></td>
         <td></td>
         <td></td>
