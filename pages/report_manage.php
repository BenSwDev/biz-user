...
<?php
        ...
        $cuponTypes = [];
        foreach(SourceList::full_list(true) as $row)
            $cuponTypes[$row['key']] = $row['fullname'];
...
?>
				<tr class="item order isSpa" data-orderid='<?=$order['orderID']?>' data-makorid='<?=$order['sourceID']?>' data-userid='<?=$order['agent_buserID']?>' data-paytypes="<?=$order['ppType']?>">
					<td class="f"><?=($order['status'] ? $order['orderIDBySite'] : "<span style='color:red'>" . $order['orderIDBySite'] . "</span>")?></td>
