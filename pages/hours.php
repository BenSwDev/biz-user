...
$custom = udb::key_row("SELECT -`periodID` AS `id`, `dateFrom` AS `dateStart`, `dateTo` AS `dateEnd`, `holidayID`, `periodName` AS `title` FROM `sites_periods` WHERE `periodType` = 0 " . ($sid == 1693 ? "" : " AND `dateTo` >= '" . date('Y-m-d') . "' ") . " AND `siteID` = " . $sid . " ORDER BY `dateEnd`", 'id');
