...
?>
				<div class="item health">
					...
					<div class="payTop">
						...
						<div class="stats">מספר הצהרה: <?=$pay['siteID']?>-<?=$pay['declareID']?></div>
					</div>
					<div class="date"><?=$pay['clientName']?></div>
					<div class="phone"><?=implode(', ', $phones)?></div>
					<div class="paytype"><div class="inner"><?=($pay["negative"]? $pay["negative"]." חריגות": "בריאות תקינה")?><?=$pay['siteOrder'] != 0?"<br /><div style=\"font-weight:normal\">הזמנה ".$pay['siteOrder']."</div>":""?></div></div>
