...
<?php
        ...
        case 'saveWeekendSpa':
            $day       = intval($_POST['day']) % 7;
            $isWeekend = intval($_POST['is']);

            if ($_CURRENT_USER->access(TfusaBaseUser::ACCESS_SUPER) < TfusaBaseUser::ACCESS_SUPER)
                throw new Exception("Cannot change that property");
            if (!$_CURRENT_USER->has($siteID))
                throw new Exception("Access denied to site " . $siteID);

            udb::query("LOCK TABLES `sites_weekly_hours` WRITE");

            $wday = udb::single_row("SELECT * FROM `sites_weekly_hours` WHERE `siteID` = " . $siteID . " AND `holidayID` = 0 AND `weekday` = " . $day);
            if ($wday)
                udb::update('sites_weekly_hours', ['isWeekend' => $isWeekend ? 1 : 0], "`siteID` = " . $siteID . " AND `holidayID` = 0 AND `weekday` = " . $day);
            else
                udb::insert('sites_weekly_hours', ['siteID' => $siteID, 'weekday' => $day, 'isWeekend' => $isWeekend]);
		...
		case 'saveWeekendSpa2':
            $day       = intval($_POST['day']) % 7;
            $isWeekend2 = intval($_POST['is']);
            ...
            $wday = udb::single_row("SELECT * FROM `sites_weekly_hours` WHERE `siteID` = " . $siteID . " AND `holidayID` = 0 AND `weekday` = " . $day);
            if ($wday)
                udb::update('sites_weekly_hours', ['isWeekend2' => $isWeekend2 ? 1 : 0], "`siteID` = " . $siteID . " AND `holidayID` = 0 AND `weekday` = " . $day);
            else
                udb::insert('sites_weekly_hours', ['siteID' => $siteID, 'weekday' => $day, 'isWeekend2' => $isWeekend2]);
...
//            $exists = udb::single_row("SELECT `holidayName`, `dateStart`, `dateEnd` FROM `notHolidays` WHERE `active` = 1 AND `dateStart` <= '" . $till . "' AND `dateEnd` >= '" . $from . "'");
//            if ($exists)
//                throw new Exception('קיים חג "' . $exists['holidayName'] . '": ' . db2date($exists['dateFrom']) . ' - ' . db2date($exists['dateTo']));
            ...
            $newID = udb::insert('sites_periods', [
                'siteID'     => $siteID,
                'periodType' => 0,
                'dateFrom'   => $from,
                'dateTo'     => $till,
            ...
            $ndays = udb::single_column("SELECT DISTINCT year.weekday FROM `year` INNER JOIN `sites_periods` ON (year.date BETWEEN sites_periods.dateFrom AND sites_periods.dateTo) WHERE periodID = " . $newID);

            udb::query("INSERT INTO `sites_weekly_hours`(`siteID`, `holidayID`, `weekday`) VALUES(" . $siteID . ", " . -$newID . ", " . implode("),(" . $siteID . ", " . -$newID . ", ", $ndays) . ")");
...
//            $exists = udb::single_row("SELECT `holidayName`, `dateStart`, `dateEnd` FROM `notHolidays` WHERE `active` = 1 AND `dateStart` <= '" . $till . "' AND `dateEnd` >= '" . $from . "' AND holidayID!=".$holidayID);
//            if ($exists)
//                throw new Exception('קיים חג "' . $exists['holidayName'] . '": ' . db2date($exists['dateFrom']) . ' - ' . db2date($exists['dateTo']));

			udb::update('sites_periods', ['dateFrom' => $from, 'dateTo' => $till, 'periodName' => $pname], '`periodID` = ' . $periodID);

            $edays = udb::single_column("SELECT DISTINCT `weekday` FROM `sites_weekly_hours` WHERE `siteID` = " . $siteID . " AND `holidayID` = " . -$periodID);
            if ($edays){
                $pdays = udb::single_column("SELECT DISTINCT year.weekday FROM `year` INNER JOIN `sites_periods` ON (year.date BETWEEN sites_periods.dateFrom AND sites_periods.dateTo) WHERE periodID = " . $periodID);

                // new days
                if ($tmp = array_diff($pdays, $edays))
                    foreach($tmp as $day)
                        udb::query("INSERT INTO `sites_weekly_hours`(`siteID`, `weekday`, `holidayID`, `isWeekend`, `extraPrice`, `packDiscount`, `openFrom`, `openTill`, `treatFrom`, `treatTill`)
                                        SELECT `siteID`, '" . $day . "', `holidayID`, `isWeekend`, `extraPrice`, `packDiscount`, `openFrom`, `openTill`, `treatFrom`, `treatTill` FROM `sites_weekly_hours` 
                                            WHERE `siteID` = " . $siteID . " AND `holidayID` = " . -$periodID . " AND `weekday` = " . reset($edays));

                // no longer relevant days
                if ($tmp = array_diff($edays, $pdays))
                    udb::query("DELETE FROM `sites_weekly_hours` WHERE `siteID` = " . $siteID . " AND `holidayID` = " . -$periodID . " AND `weekday` IN (" . implode(',', $tmp) . ")");
            }
        ...
        case 'removePeriod':
            ...
            udb::query("DELETE FROM `rooms_min_nights` WHERE `periodID` = " . $periodID);
            udb::query("DELETE FROM `rooms_prices` WHERE `periodID` = " . $periodID);
            udb::query("DELETE FROM `sites_weekly_hours` WHERE `siteID` = " . $siteID . " AND `holidayID` = " . -$periodID);
            udb::query("DELETE FROM `sites_periods` WHERE `periodType` = 0 AND `periodID` = " . $periodID);
            $result['status'] = 0;
			break;

        case 'masterExtraAdd':
            if ($_POST['dateAdd'])
                $_POST['dateAdd'] = date2db($_POST['dateAdd']);
            ...
            $master = udb::single_row("SELECT * FROM `therapists` WHERE `therapistID` = " . $input['tid'] . " AND `deleted` = 0");

            if (!$input['tid'] || !$master)
                throw new Exception("מטפל לא נמצא");
            if(!$_CURRENT_USER->has($master['siteID']))
                throw new Exception("אינך רשאי לעשות שינוים למטפל זה");
            if (!$input['dateAdd'])
                throw new Exception("נא להכניס תאריך");
            if (!$input['sumAdd'])
                throw new Exception("נא להכניס סכום");

            udb::insert('therapists_pay_extra', [
                'therapistID' => $master['therapistID'],
                'date'        => $input['dateAdd'],
                'sum'         => $input['multiplier']*round($input['sumAdd'], 1),
                'agent'       => $_CURRENT_USER->id(),
                'description' => $input['descAdd'] ?? ''
            ]);
