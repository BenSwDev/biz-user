...
$_timer->log();
        ...
        if (!$_CURRENT_USER->has($order['siteID']) || (!$order['status'] && $order['apiSource'] == 'spaplus' && $order['sourceID'] == 'spaplus'  && substr_count($order['onlineData'], 'timeout') > 0))
...
$_timer->log();
        ...
        $que = "SELECT orders.*, health_declare.guid as health_guid, therapists.siteName AS `masterName`, rooms_units.unitName AS `roomName`, treatments.treatmentName
                FROM `orders` 
                    LEFT JOIN `orderUnits` USING(`orderID`) LEFT JOIN `rooms_units` USING(`unitID`)
...
$_timer->log();
    ...
    $allSources = SourceList::site_list($siteID, false, $order['sourceID']);
    $jssourcesArray = "";
