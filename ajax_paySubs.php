...
if ($_POST['orderID'] && !$_POST['subID'])
    $_POST['subID'] = $_POST['orderID'];
...
if ($subID){
    $order = udb::single_row("SELECT * FROM `subscriptions` WHERE `subID` = " . $subID . " AND `siteID` IN (" . $_CURRENT_USER->sites(true) . ")");
    if (!$order)
...
$user_log = new UserActionLog($_CURRENT_USER, $siteID, $subID);
...
try {
    ...
    switch($_POST['act']){
        case 'initPay':
            ...
            if (!$subID)
                throw new Exception('מספר מנוי שגוי');
            ...
            if ($input['via'] == 'ccard'){
                ...
                $req = [
                    '_target'      => 'subscription',
                    'customerName' => $input['iname'] ?: $order['clientName'],
                ...
                if ($input['type'] == 2){
                    ...
                    $trans = $client->initFrameCardTest($getToken, $req);

                    udb::insert('subscriptionPayments', [
                        'payType'   => 'ccard',
                    ...
                    $req['description'] = 'תשלום עבור מנוי ' . prettySubsNumber($order['subNumber']);
            ...
            elseif ($input['via'] == 'guest' && $protel){
                ...
                if (strcasecmp($protelOrder['customerName'], $input['booker']))
                    throw new Exception("שם המזמין לא נכון להזמנה שבחרת");
                ...
                $input['_target'] = 'subscription';
                $input['desc']    = $description = 'תשלום עבור מנוי ' . prettySubsNumber($order['subNumber']);

                // creating payment row
                $payID = udb::insert('subscriptionPayments', [
                    'payType'    => $realType,
                    'buserID'    => $_CURRENT_USER->id(),
                    'subID'      => $subID,
                    'startTime'  => date('Y-m-d H:i:s'),
                ...
                $prToken = ['CHARGE-SUBS' , $protelOrder['protelOrderID'], $protelOrder['innerID'], $subID, $payID];
                ...
                udb::update('subscriptionPayments', ['paymentID' => 1000000 + $logID], "`lineID` = " . $payID);
                ...
                do {
                    ...
                    $resp = udb::single_row("SELECT `resultData`, `complete` FROM `subscriptionPayments` WHERE `lineID` = " . $payID . " AND `endTime` IS NOT NULL AND `resultData` IS NOT NULL");
                    if ($resp || ++$try >= 5)
                        break;
                } while(1);
            ...
            // payment NOT by credit card
                ...
                if ($input['via'] == 'vouchers'){
                    ...
                    $rd['couponUse'] = $coupon->charge($input['sum'], 'מנוי - ' . prettySubsNumber($order['subNumber']));
                ...
                if ($noInvoice)
                    udb::update('subscriptionPayments', ['invoice' => '-'], "`lineID` = " . $payID);
                ...
                if (!$noInvoice && $canInvoice && $spTypes[$input['via']] == 2){
                    try {
                        $invData = [
                            ...
                            'desc'      => 'תשלום עבור מנוי ' . prettySubsNumber($order['subNumber'])
                        ];
                        ...
                        udb::update('subscriptionPayments', [
                            'paymentID'  => $res['_transID'] ?: 0,
                            'invoice'    => $res['invoiceURL'] ?: '-',
                            'resultData' => json_encode($rd, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE)
                        ], '`lineID` = ' . $payID);

                        $user_log->save('subs_payment_invoice', $siteID, $subID, ['lineID' => $payID]);
                    }
                    catch(Exception $e){
        ...
        case 'directPay':
            ...
            $payment = udb::single_row("SELECT * FROM `subscriptionPayments` WHERE `subID` = " . $subID . " AND `lineID` = " . $payID);
            ...
            if (!$payment)
                throw new Exception('Cannot find payment for booking #' . $subID);
            ...
            $description = 'תשלום עבור מנוי ' . prettySubsNumber($order['subNumber']);

            $res = $client->directPay($input['sum'], $description, $payData['tokenData'], $payID, $input['pays'] ?: 1);
            if (!$res['success'])
            ...
            $lineID = udb::insert('subscriptionPayments', $payque);

//            if ($res['phone']){
//                $siteName = udb::single_value("SELECT sites.siteName FROM `sites` INNER JOIN `subscriptions` USING(`siteID`) WHERE subID = " . $subID);
//                sms_payment_success($res, $siteName, $res['phone']);
//            }

            $user_log->save('subscription_payment', $siteID, $subID, ['lineID' => $lineID]);
            break;
        ...
        case 'directRefund':
            ...
            $res = $client->directRefund($input['sum'], 'זיכוי מנוי ' . prettySubsNumber($order['subNumber']), $payData['tokenData'], $payID, 1);
            if (!$res['success'])
                throw new Exception(($res['ccode'] ? 'שגיאה ' . $res['ccode'] : '') . ' ' . $res['error']);
            ...
            $user_log->save('subscription_refund', $siteID, $subID, ['lineID' => $payID, 'payType' => $payment['payType'], 'sum' => $input['sum'], 'endTime' => $payment['endTime'], 'newLineID' => $lineID]);
            break;
        ...
        case 'payDelete':
            ...
            if ($payment['payType'] == 'coupon' && $payment['provider'] == 'vouchers'){
                ...
                $source = 'subscription #' . $payment['subID'];
            ...
            elseif ($payment['payType'] == 'guest' && $payment['provider'] == 'guest' && $payment['paymentID'] > 1000000){
                ...
                // creating payment row
                $refundID = udb::insert('subscriptionPayments', [
                    'payType'    => 'refund',
                    'buserID'    => $_CURRENT_USER->id(),
                    'subID'      => $subID,
                    'startTime'  => date('Y-m-d H:i:s'),
                ...
                $prToken = ['CHARGE-SPA' , $protelOrder['protelOrderID'], $protelOrder['innerID'], $subID, $refundID];
                ...
                udb::update('protel_comm_log', ['response' => $response], '`id` = ' . $logID);
                udb::update('subscriptionPayments', ['paymentID' => 1000000 + $logID], "`lineID` = " . $refundID);
                ...
                do {
                    ...
                    $resp = udb::single_row("SELECT `resultData`, `complete` FROM `subscriptionPayments` WHERE `lineID` = " . $refundID . " AND `endTime` IS NOT NULL AND `resultData` IS NOT NULL");
                    if ($resp || ++$try >= 5)
                        break;
                } while(1);
                ...
                udb::update('subscriptionPayments', ['cancelData' => json_encode($res, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE)], '`lineID` = ' . $payID);
            }
            elseif ($payment['invoice']){
                try {
                    ...
                    $res = $client->sendPrintout($payment['payType'], $payment['sum'], 'זיכוי תשלום עבור מנוי ' . prettySubsNumber($order['subNumber']), $payData, $payID, $client->refund_invoice_type(), true);
                    ...
                    $lineID = udb::insert('subscriptionPayments', $resque);
                }
                catch (Exception $e){
            ...
            if (!$payment['invoice'])
                udb::query("DELETE FROM `subscriptionPayments` WHERE `payType` NOT IN ('ccard', 'refund') /*AND (`payType` <> 'coupon' OR `provider` <> 'vouchers')*/ AND `subID` = " . $subID . " AND `lineID` = " . $payID);

            updateOrderAdvance($subID);
            $user_log->save('subscription_pay_remove', $siteID, $subID, ['lineID' => $payment['lineID'], 'payType' => $payment['payType'], 'sum' => $payment['pay'], 'endTime' => $payment['endTime']]);
            break;
        ...
        case 'payRefund':
            ...
            if (!$payment['complete'] || !$payment['endTime'])
                throw new Exception('Payment failed or was not completed');
            if (strcasecmp($payment['payType'], 'ccard') || $payment['subType'] == 'card_test')        // if not cc or wasn't payment
            ...
            if ($_POST['act'] == 'payCancel'){
                ...
                if ($res['success']){
                    ...
                    udb::update('subscriptionPayments', ['cancelled' => 1, 'cancelData' => json_encode($res, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE)], '`lineID` = ' . $payID);

                    $user_log->save('subscription_pay_cancel', $siteID, $subID, ['lineID' => $payID, 'payType' => $payment['payType'], 'sum' => $payment['sum'], 'endTime' => $payment['endTime']]);
                }
            ...
            if ($_POST['act'] == 'payRefund') {
                ...
                $user_log->save('subscription_pay_refund', $siteID, $subID, ['lineID' => $payID, 'payType' => $payment['payType'], 'sum' => $payment['sum'], 'endTime' => $payment['endTime'], 'newLineID' => $lineID]);
            }
        ...
        case 'getInvoice':
            ...
            $payment = udb::single_row("SELECT * FROM `subscriptionPayments` WHERE `subID` = " . $subID . " AND `lineID` = " . $payID);
            if ($payment['invoice'])
                throw new Exception("כבר נוצרה חשבונית עבור תשלום זה");
            ...
            $user_log->save('subscription_pay_invoice', $siteID, $subID, ['lineID' => $payID]);
        ...
        case 'directPayPop':
            ...
            ob_start();
?>
<div class="popup pop-pay payAmount" id="payAmount" data-order="<?=$subID?>" data-pid="<?=$payID?>" style="display:block">
...
<?php
        ...
        case 'invoicePop':
            ...
            $desc = 'מנוי ' . prettySubsNumber($order['subNumber']) . ' - ' . htmlspecialchars($site['siteName']);
?>
<div class="popup pop-pay payAmount" id="payAmount" data-order="<?=$subID?>" data-pid="<?=$payID?>" style="display:block">
    <div class="popup_container">
        <div class="close" onclick="$('#payAmount').fadeOut('fast')"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 21 21" width="21" height="21"><path class="shp0" d="M1.3 1.3C1.8 0.9 2.5 0.9 2.9 1.3L11 9.4 19.1 1.3C19.5 0.9 20.2 0.9 20.7 1.3 21.1 1.8 21.1 2.5 20.7 2.9L12.6 11 20.7 19.1C21.1 19.5 21.1 20.2 20.7 20.7 20.4 20.9 20.2 21 19.9 21 19.6 21 19.3 20.9 19.1 20.7L11 12.6 2.9 20.7C2.7 20.9 2.4 21 2.1 21 1.8 21 1.5 20.9 1.3 20.7 0.9 20.2 0.9 19.5 1.3 19.1L9.4 11 1.3 2.9C0.9 2.5 0.9 1.8 1.3 1.3Z"></path></svg></div>
        ...
        <div class="form">
            <div class="inputWrap full">
                <input type="text" inputmode="numeric" name="tz" id="tZehoot" value="<?=$order['clientTZ']?>" />
                <label for="tZehoot">תעודת זהות</label>
            </div>
            <div class="inputWrap full">
                <input type="text" name="full_name" id="full_name" value="<?=str_replace('"', '&quot;', $order['clientName'])?>" />
            ...
            <div class="inputWrap full">
                <input type="email" name="email" id="email" value="<?=$order['clientEmail']?>" />
...
?>
            <div class="submit" onclick="getInvoiceSubs()">הפקת חשבונית
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 34 26" width="34" height="26"><path class="shp0" d="M32 7.7L16.3 7.7 16.3 2 2 13 16.3 24 16.3 18.3 32 18.3 32 7.7Z"></path></svg>
            </div>
        </div>
    </div>
...
<?php
        ...
        default:
            ...
            $que = "SELECT * FROM `subscriptions` WHERE `subID` = " . $subID;
            $order = udb::single_row($que);

            if (!$order)
                throw new Exception('Cannot find subscription #' . $subID);
            if(!$_CURRENT_USER->has($order['siteID']))
                throw new Exception("מנוי הזה לא שייך למשתמש");
            ...
            $payments  = udb::single_list("SELECT * FROM `subscriptionPayments` WHERE (`payType` <> 'ccard' OR `subType` = 'direct' OR `complete` = 1) AND `cancelled` = 0 AND `subID` = " . $subID . " ORDER BY `lineID`");

            $alreadyPaid = 0;
            $yet2pay     = round($order['price'] - $order['discount'], 2);
            //$adv2pay     = $order['advance'];

            foreach($payments as $pay){
                if ($pay['subType'] == 'card_test' || $pay['cancelData'] || !$pay['complete'])
                    continue;
...
//                    $adv2pay = max(0, $adv2pay - $pay['sum']);
            ...
            ob_start();
?>
<div class="pay_order pop-pay  order" id="pay_orderPop" data-order-id="<?=$subID?>">
    <div class="container">
        ...
        <div class="close" onclick="closePayOrder(<?=($reload ? $subID : '')?>)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 21 21" width="21" height="21"><path class="shp0" d="M1.3 1.3C1.8 0.9 2.5 0.9 2.9 1.3L11 9.4 19.1 1.3C19.5 0.9 20.2 0.9 20.7 1.3 21.1 1.8 21.1 2.5 20.7 2.9L12.6 11 20.7 19.1C21.1 19.5 21.1 20.2 20.7 20.7 20.4 20.9 20.2 21 19.9 21 19.6 21 19.3 20.9 19.1 20.7L11 12.6 2.9 20.7C2.7 20.9 2.4 21 2.1 21 1.8 21 1.5 20.9 1.3 20.7 0.9 20.2 0.9 19.5 1.3 19.1L9.4 11 1.3 2.9C0.9 2.5 0.9 1.8 1.3 1.3Z"></path></svg></div>
        <div class="title mainTitle">ביצוע תשלום</div>
        <form class="form" id="payForm" action="" method="post" autocomplete="off" >
            <input type="hidden" name="subID" value="<?=$order['subID']?>" id="orderForm-orderID" />
            <!-- div></div -->
            <div class="inputWrap three paypp">
                <input type="text" value="<?=round($order['price'] - $order['discount'], 2)?>" name="total_pay" id="total_pay" class="" readonly />
            ...
            <div class="payMethod">
                ...
                <input type="radio" name="paymethod" value="2" id="safecard" />
                <label for="safecard"><?=($canCharge ? 'כרטיס לערבון' : 'בדיקת כרטיס')?></label>

                <div class="inputLblWrap full" style="opacity:0; pointer-events:none; height:0">
                    <label class="switch" for="approved">
                        <input type="checkbox" name="approved" id="approved" value="1" data-on="0" data-off="<?=$yet2pay?>" onclick="$('#payamount').val(this.checked ? this.dataset.on : this.dataset.off)" />
...
?>
                ...
                <div class="inputWrap half manuy-number" style="display:none">
                    <input type="text" name="manuy-number" id="manuy-number"  />
...
?>
                ...
                <div class="cc-left-arrow" onclick="initPay($(this).closest('.pay_order'), 'ajax_paySubs.php')"><span>בדיקת כרטיס</span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 34 26" width="34" height="26"><path class="shp0" d="M32 7.7L16.3 7.7 16.3 2 2 13 16.3 24 16.3 18.3 32 18.3 32 7.7Z"></path></svg></div>
<?php
            if ($canInvoice) {
?>
                <div class="inputWrap half invoice-name">
                    <input type="text" name="invoiceName" id="invoiceName" value="<?=str_replace('"', '\"', $order['clientName'])?>" />
...
<?php
            if (count($payments)){
                ...
                foreach($payments as $pay){
                    $payType = ($pay['payType'] == 'ccard' && $pay['subType'] == 'card_test') ? 'בדיקת כרטיס' : UserUtilsNew::method($pay['payType'], $pay['provider']);
...
?>
                <div class="item payment <?=$payClass ?>" data-pay-id="<?=$pay['lineID']?>" data-tps="<?echo $pay['payType'], $pay['provider'];?>">
                    ...
                    <div class="amount">₪<?=round($pay['sum'], 1)?></div>
...
<?php
                    ...
                    elseif ($pay['payType'] == 'ccard'){
                        // card for security
                        if ($pay['subType'] == 'card_test' && $pay['token'] && $canCharge){
?>
