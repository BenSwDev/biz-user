...
try {
    ...
    $siteID = udb::single_value("SELECT `siteID` FROM `subscriptions` WHERE `subID` = " . $orderID);
    if (!$_CURRENT_USER->has($siteID))
        throw new Exception("Access denied to subscription #" . $orderID);

    $payment = udb::single_row("SELECT * FROM `subscriptionPayments` WHERE `subID` = " . $orderID . " AND `lineID` = " . $payID);
    if (!$payment)
        throw new Exception('Cannot find payment for subscription #' . $orderID);
