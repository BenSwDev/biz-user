...
//                $pays[$refund['lineID']]['_rtype'] = $_type;
    ...
    $refunds = udb::single_list("SELECT p.lineID, p.payType, t.sum FROM `gift_purchase_payments` AS `p` INNER JOIN `gifts_purchases` AS `o` ON (p.orderID = o.pID) INNER JOIN `pm_transactions` AS `t` ON (t.transID = o.refunded) WHERE " . str_replace('p.startTime', 't.createTIme', implode(' AND ', $where3)) . " AND t.status = 1");
    foreach($refunds as $refund){
