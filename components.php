...
function orderComp($order){
    ...
    if (!$order['status'] && $order['apiSource'] == 'spaplus' && $order['sourceID'] == 'spaplus' && substr_count($order['onlineData'], 'timeout') > 0){
        emptyOrderComp($order);
        return;
    }
...
function emptyOrderComp($order)
{
    ...
    $multiCompound = !$_CURRENT_USER->single_site;
?>
    <div class="item box-order order canceled isSpa new is-lead" data-orderid="<?=$order['orderID']?>">
        <?if($order['sourceID']){
            ?>
            <div class="domain-icon <?=$order['sourceID']?>" title='<?=$order['sourceID']?>' style="background-color: <?=$sourcesArray[$order['sourceID']]['hexColor']?>"><?=$sourcesArray[$order['sourceID']]['letterSign']?></div>
        <?}else{?>
        ...
        <ul class="f" style="min-height:250px;display:flex;align-items:center;justify-content:center;font-size:16px">
			  הזמנת אונליין שלא הושלמה
        </ul>
    </div>
<?php
}
