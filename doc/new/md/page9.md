...
#  SpaPlusRelay
 ...
 עם עדכון הערכים, מתבצעת קריאה למערכת חיצונית (`SpaPlusRelay`), לצורך עדכוני מחירים בזמן אמת.


להלן כל המידע המופיע בקובץ שהועבר (pages/treatments.php) ואשר נוגע ל‑SpaPlusRelay (הנקרא לעיתים גם "psa plus relay" בשאלתך). המידע מוצג במלואו בהתאם למה שנמצא בקובץ, ללא הוספת פרטים שאינם מצוינים בו. בסוף מוצגות שתי טבלאות – האחת מיועדת למשתמש והאחרת למתכנת – ובהן סיכום התרחיש הרלוונטי הקשור ל‑SpaPlusRelay.

---

# מה מופיע בקובץ לגבי SpaPlusRelay?

1. בתוך פונקציית הטיפול ב־POST (כאשר משתמש בעל הרשאות מתאימות שומר מחירים חדשים), מתבצעת יצירת מופע (instance) של **SpaPlusRelay** עם מזהה האתר \($sid\).

2. מיד לאחר יצירת המופע, מתבצעת קריאה לפונקציה:
   ```php
   $relay->sendPrices();
   ```
   שתפקידה לשלוח את עדכוני המחירים לגורם חיצוני (SpaPlus).

3. במידה וזרק החריג (Exception) בעת שליחת המחירים, נשלח אימייל לכתובת
   ```text
   ...
   ```
   ותוכן ההודעה כולל את הודעת השגיאה שהתקבלה, בצירוף מזהה האתר בעבורו בוצעה השליחה.
...
| תרחיש                         | מה קורה בפועל                                                                                                                                                                                         |
|------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| שמירת מחירי טיפולים במערכת   | בעת שהמשתמש שומר מחירים חדשים לטיפולים (לאחר שמתקיים תנאי ההרשאות) מתבצעת קריאה ל‑SpaPlusRelay לצורך שליחת המידע המעודכן ל‑SpaPlus.                                                                 |
| כישלון בשליחת המחירים ל‑SpaPlus | אם נוצרת תקלה בעת שליחת המחירים (Exception), המערכת שולחת אימייל אוטומטי לכתובת alchemist.tech@gmail.com ומדווחת על הכישלון ועל הודעת השגיאה שחזרה, יחד עם מזהה האתר שבו נעשה העדכון.                                        |
...
| חלק בקוד                                                | הסבר טכני                                                                                                                                                                           |
|---------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ```php<br>try {<br>    $relay = new SpaPlusRelay($sid);<br>    $relay->sendPrices();<br>} catch (Exception $e) {<br>    mail(...);<br>}<br>``` | 1. יצירת אובייקט `SpaPlusRelay` תוך העברת מזהה האתר כפרמטר.<br>2. קריאה לפונקציה `sendPrices()` עבור שליחת המחירים המעודכנים ל‑SpaPlus.<br>3. טיפול בחריגות: במקרה של שגיאה, נשלח מייל עם הפרטים הדרושים. |

---

**חשוב להדגיש**: כל המידע לעיל לקוח ישירות מהקובץ pages/treatments.php. לא מופיעים בקובץ פרטים נוספים על אופן הפעולה הפנימי של SpaPlusRelay, על הממשק (API) המדויק שלו, או על הכתובות שאליהן הוא שולח את הנתונים, מעבר למה שמוצג כאן.
