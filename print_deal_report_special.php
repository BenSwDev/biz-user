...
<table style="width:100%;max-width:900px;text-align:center;">
	<tr>
		<td>
			<div style='font-size:18px;font-weight:bold'>דוח ריכוז הכנסות</div>
...
<table style='width:300px'>
	<tr><td style='text-align:center' colspan=2><b>פירוט הכנסות</b></td></tr>
<?


UserUtilsNew::init($siteID);

$where = $where2 = $where3 = ["p.complete = 1", "o.siteID = " . $siteID, 'date' => "p.startTime BETWEEN '" . $timeFrom . " 00:00:00' AND '" . $timeTill . " 23:59:59'", 'vou' => "(p.payType <> 'coupon' OR p.provider NOT IN ('" . implode("','", array_merge($ignoreAll, $byTreatDate)) . "'))", 'sub' => "p.payType NOT IN ('member', 'member2')"];
...
$totalQue = "SELECT `payType`, SUM(`sum`) AS `total`, COUNT(*) AS `cnt`
             FROM (
                ...
                WHERE " . str_replace('p.startTime', 't.createTIme', implode(' AND ', $where3)) . " AND t.status = 1)
             ) AS `bigT`
             GROUP BY `payType` 
             ORDER BY NULL";
...
if ($totals['coupon']['total']){
    ...
    $where['pt'] = $where2['pt'] = "p.payType = 'coupon' AND p.provider NOT IN ('" . implode("','", array_merge($ignoreAll, $byTreatDate)) . "')";
...
//---------- special case for some payments - must use "treatment date" of active orders instead of "payment date" --------------------
    $coupWhere = array_merge($where, ['date' => "sub.timeFrom BETWEEN '" . $timeFrom . " 00:00:00' AND '" . $timeTill . " 23:59:59'", 'pt' => "p.payType = 'coupon' AND p.provider IN ('" . implode("','", $byTreatDate) . "')"]);

    $que = "SELECT `provider`, SUM(`sum`) AS `total`, COUNT(*) AS `cnt`
        FROM (
            SELECT p.lineID, p.provider, p.sum, MIN(sub.timeFrom) 
            FROM `orderPayments` AS `p` INNER JOIN `orders` AS `o` USING(`orderID`) INNER JOIN `orders` AS `sub` ON (sub.parentOrder = o.orderID AND sub.orderID <> o.orderID) 
            WHERE " . implode(' AND ', $coupWhere) . " AND p.cancelled = 0
            GROUP BY p.lineID
            ORDER BY NULL
        ) AS `tmp`
        WHERE 1
        GROUP BY tmp.provider
        ORDER BY NULL";
    $byTreat = udb::single_list($que);

    foreach($byTreat as $spaplus){
        $subTotals[$spaplus['provider']] = $spaplus;

        $totals['coupon']['total'] += $spaplus['total'];
        $totals['coupon']['cnt'] += $spaplus['cnt'];
    }
    unset($spaplus, $coupWhere, $byTreat);
//----------  end of special case ---------------
}
